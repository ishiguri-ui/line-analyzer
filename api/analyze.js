...
export default async function handler(req, res) {
  ...
  const configs = {
    ...
    night_staff:    { role: '人間関係の専門家', focus: `飲み仲間・飲み友達（${genderText}）の感情状態・何を求めているかを読み取り、気軽で自然な友人としての返信を3パターン提案してください。お酒の場での会話のノリや距離感を意識してください。` },
    regular_staff:  { role: '夜職・接客業に詳しい恋愛コミュニケーション専門家', focus: `行きつけのお店のキャスト・スタッフ（${genderText}）のLINEを分析してください。営業トークと本音を見極め、相手との距離を自然に縮めて特別な存在になるための返信を3パターン提案してください。重くなりすぎず、また軽すぎず、また来たいと思わせつつ相手にも好意を持ってもらえるバランスを意識してください。` },
  ...
  const relNames = {
    ...
    night_customer: 'お客様（飲食店）', night_staff: '飲み友達', regular_staff: '行きつけのお店の人', other: 'その他'
  };
