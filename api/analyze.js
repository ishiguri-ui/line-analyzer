...
export default async function handler(req, res) {
  ...
  // jfifなど非標準形式をjpegに正規化
  const normalizedMediaType = (mediaType || 'image/jpeg')
    .replace('image/jfif', 'image/jpeg')
    .replace('image/jpg', 'image/jpeg');
  const safeMediaType = ['image/jpeg','image/png','image/gif','image/webp'].includes(normalizedMediaType)
    ? normalizedMediaType : 'image/jpeg';

  // HEIC等の非対応形式はjpegとして扱う
  const safeMediaType = (mediaType && ['image/jpeg','image/png','image/gif','image/webp'].includes(mediaType))
  ...
  const relationshipConfig = {
    ...
    crush: { role: '恋愛コーチ', focus: `付き合う前の相手（${genderText}）の脈あり・なしのサイン・好意の度合いを分析し、好印象を与えてより親密になれる返信を3パターン提案してください。` },
    ...
    night_customer: { role: '夜職・ホスト・キャバクラ業界のコミュニケーション専門家', focus: `夜職のお客様（${genderText}）の本音・感情・お店への関心度を読み取り、来店意欲を高めて関係を深めるLINE返信を3パターン提案してください。営業感を出しすぎずに自然に距離を縮めることを意識してください。` },
    night_staff: { role: '夜職・ホスト・キャバクラ業界のコミュニケーション専門家', focus: `夜職のお店の子（${genderText}）の感情・本音・状況を読み取り、良好な関係を築くための自然な返信を3パターン提案してください。` },
）の意図・要求・感情を読み取り、信頼関係を築くビジネス的な返信を3パターン提案してください。` },
    other: { role: 'コミュニケーションの専門家', focus: `相手（${genderText}）の感情と意図を読み取り、状況に最適な返信を3パターン提案してください。` }
  };

  const relNames = { friend:'友達', crush:'恋愛関係（付き合う前）', lover:'恋愛関係（恋人）', couple:'家族（夫婦）', family:'家族（子ども・両親）', night_customer:'お客様（飲食店）', night_staff:'飲み友達',
 other:'その他' };
  ...
  const choiceSection = choiceHistory ? `\n【過去の返信選択履歴（この人に刺さった返信のパターン）】\n${choiceHistory}\n※上記の傾向を踏まえて返信案を提案してください。\n` : '';
